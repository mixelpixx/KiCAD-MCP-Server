...
*Backlog spec, opened 2026-07-24. Proven value this session: AT-SPI both (a) demonstrated
...
dialogs, plugin buttons) to complement `kipy` (design-content only).*
...
`kipy` manipulates the *design* (board/schematic objects). It does NOT touch GUI chrome —
...
a helper running *inside* KiCad works on Linux/Windows/macOS:
...
- BUT `plugin.json` **is** the declarative UI definition for API plugins (actions, buttons,
  scopes) — reading it on disk tells you what *should* register (that's how the button bug was
...
- `kicad_gui_tree()` → the clickable widget tree (roles + names), so an agent can *see* the GUI.
