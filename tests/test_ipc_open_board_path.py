...
def test_preserves_windows_project_path_semantics(monkeypatch: Any) -> None:
    """A Windows path stays Windows-style even when tests run on another OS."""
    _install_fake_kipy(monkeypatch)
    backend = _make_backend(
        [_FakeDocumentSpecifier("DankroRunner.kicad_pcb", r"C:\Users\u\proj\pcb")]
    )

    assert backend.get_open_board_path() == r"C:\Users\u\proj\pcb\DankroRunner.kicad_pcb"
