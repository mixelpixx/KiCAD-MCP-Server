...
Ported from src/prompts/footprint.ts.
"""
...
def create_footprint_guide(
    component: Annotated[
        str,
        Field(
            description=(
                "Component description, e.g. 'SOT-23 NPN transistor' or "
                "'2-pin JST XH 2.5mm connector'"
            )
        ),
    ],
    libraryPath: Annotated[
        Optional[str], Field(description="Target .pretty library path (optional)")
    ] = None,
) -> str:
    """Create a correct KiCAD 9 footprint for a component using the create_footprint tool."""
...
def footprint_ipc_checklist(
    footprintPath: Annotated[
        str, Field(description="Path to the .kicad_mod file to review")
    ],
) -> str:
    """Review a footprint against IPC-7351 land pattern guidelines."""
    return f"""Review the footprint at {footprintPath} against IPC-7351 land pattern guidelines.
