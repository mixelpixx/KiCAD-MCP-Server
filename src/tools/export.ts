...
export function registerExportTools(server: McpServer, callKicadScript: CommandFunction): void {
  ...
  server.tool(
    "export_netlist",
    {
      ...
      format: z.enum(["kicadsexpr", "kicadxml", "cadstar", "orcadpcb2", "spice"]).optional().describe("Netlist format (default: kicadsexpr)")
    },
  ...
  server.tool(
    "export_position_file",
    {
      ...
      format: z.enum(["csv", "ascii", "gerber"]).optional().describe("File format (default: csv)"),
      units: z.enum(["mm", "in"]).optional().describe("Units to use (default: mm)"),
      side: z.enum(["front", "back", "both"]).optional().describe("Which board side to include (default: both)")
    },
